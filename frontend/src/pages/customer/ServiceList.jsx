...
const ServiceList = () => {
  ...
  const getRatingStars = (rating) => {
    return Array.from({ length: 5 }, (_, i) => (
      <span key={i} className={`material-symbols-outlined text-sm sm:text-base ${i < rating ? 'text-amber-400 fill-current' : 'text-slate-300'}`}>
        star
      </span>
    ));
  };
          ...
          <div className="flex flex-col gap-4 sm:gap-6">
            <div className="flex items-center gap-3 sm:gap-4">
              <div className="w-12 h-12 sm:w-16 sm:h-16 lg:w-20 lg:h-20 bg-white/20 backdrop-blur-sm rounded-xl sm:rounded-2xl flex items-center justify-center shadow-2xl flex-shrink-0">
                <span className="material-symbols-outlined text-2xl sm:text-4xl lg:text-5xl text-white">{getCategoryIcon()}</span>
              </div>
              <div className="flex-1 min-w-0">
                ...
                <p className="text-teal-100 text-xs sm:text-sm lg:text-base">Find expert technicians • Compare prices • Book instantly</p>
              </div>
            </div>
        ...
        {loading ? (
          <div className="flex items-center justify-center py-12 sm:py-20">
            <div className="text-center">
              ...
              <p className="text-slate-600 font-medium text-sm sm:text-base">Finding the best technicians for you...</p>
            </div>
          </div>
                ...
                >
                  ...
                  <div className="relative bg-gradient-to-br from-[#1F7F85] to-[#0F4C5C] p-4 sm:p-6 pb-12 sm:pb-16">
                    <div className="absolute top-3 sm:top-4 right-3 sm:right-4 bg-white/20 backdrop-blur-md rounded-full px-2 sm:px-3 py-1 sm:py-1.5 flex items-center gap-1 sm:gap-1.5 shadow-lg">
                      <span className="material-symbols-outlined text-amber-300 fill-current text-sm sm:text-lg">star</span>
                      <span className="text-white font-bold text-xs sm:text-sm">{service.technician?.rating || 4.5}</span>
                    </div>
                  ...
                  <div className="p-4 sm:p-6 -mt-6 sm:-mt-8 relative">
                    <div className="bg-white rounded-lg sm:rounded-xl shadow-md p-3 sm:p-4 mb-3 sm:mb-4 border border-slate-100">
                      <div className="flex items-center gap-2 mb-1.5 sm:mb-2">
                        <span className="material-symbols-outlined text-[#1F7F85] text-base sm:text-xl">build</span>
                        <p className="text-[10px] sm:text-xs font-semibold text-slate-500 uppercase tracking-wide">Service Type</p>
                      </div>
                      <p className="text-xs sm:text-sm font-bold text-[#0F4C5C] capitalize truncate">{service.serviceType}</p>
                    </div>

                    <div className="space-y-2 sm:space-y-3 mb-3 sm:mb-4">
                      <div>
                        <p className="text-[10px] sm:text-xs font-semibold text-slate-500 mb-1 sm:mb-1.5 flex items-center gap-1">
                          <span className="material-symbols-outlined text-xs sm:text-sm">description</span>
                          Description
                        </p>
                        <p className="text-xs sm:text-sm text-slate-700 line-clamp-2 leading-relaxed">{service.description}</p>
                      </div>
                      
                      <div className="flex items-start gap-2 text-[10px] sm:text-xs text-slate-500 bg-slate-50 rounded-lg p-2 sm:p-2.5">
                        <span className="material-symbols-outlined text-sm sm:text-base text-[#1F7F85] mt-0.5 flex-shrink-0">location_on</span>
                    ...
                    <div className="flex items-center justify-between pt-3 sm:pt-4 border-t-2 border-slate-100 gap-3">
                      <div>
                        <p className="text-[10px] sm:text-xs text-slate-500 font-medium mb-0.5">Starting from</p>
                  ...
                  <div className="absolute top-2 left-2 bg-emerald-500 text-white text-[9px] sm:text-[10px] font-bold px-1.5 sm:px-2 py-0.5 sm:py-1 rounded-full flex items-center gap-0.5 sm:gap-1 shadow-lg">
                    <span className="material-symbols-outlined text-[10px] sm:text-xs fill-current">verified</span>
                    VERIFIED
                  </div>
                </div>
              ))}
          ...
          <div className="text-center py-12 sm:py-20 px-4">
            <div className="w-16 h-16 sm:w-24 sm:h-24 bg-slate-100 rounded-full flex items-center justify-center mx-auto mb-3 sm:mb-4">
              <span className="material-symbols-outlined text-3xl sm:text-5xl text-slate-400">search_off</span>
            </div>
            <h3 className="text-lg sm:text-xl font-bold text-slate-700 mb-2">No services found</h3>
            <p className="text-sm sm:text-base text-slate-500">Try adjusting your search or check back later</p>
          </div>
        )}
      </div>
    </div>
